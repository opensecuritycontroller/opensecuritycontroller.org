...
## Introduction
The following work example will describe how to develop and assemble an OSC plugin using Maven, Eclipse and the Bndtools plugin for [Eclipse](http://bndtools.org). The plugin implementation will be created as a single OSGi bundle, and then packaged up including its dependencies in a plugin archive suitable for deployment into the OSC server.


## Creating a Maven Project
OSGi bundles can be easily created using a standard maven project, just like you’d use to create a JAR file. Importantly the project needs to add a build plugin to generate the necessary OSGi metadata. Plugin implementors can therefore start by creating a simple maven project. In this example we assume that the plugin project has the group id org.osc.example, and the artifact id `example-manager-impl`.
...
```
The bnd-maven-plugin accepts further configuration from a bnd.bnd file, or included directly in the pom as CDATA. Using a bnd.bnd file is preferred as the Bndtools plugin provides a convenient editor and additional validation. If no special configuration instructions are needed then no bnd file needs to be created. 

### Setting up Maven Dependencies
In addition to using the bnd-maven-plugin it is important to be careful when scoping the dependencies for your OSGi bundle project. In many Maven builds large numbers of project dependencies are added with no thought for the scope at which they should be included.  
Correctly scoping a dependency is a relatively simple process, simply follow the steps below:  

1. Is the dependency for an API provided by the platform, for example the OSGi framework API, or the OSC plugin API? If so then the scope is `provided`.
2. Is the dependency going to be repackaged inside the bundle being built by this project? If so then the scope is `provided`.
3. Is the dependency for a build-time annotation (e.g. OSGi’s @Component)? If so then the scope is `provided`.
4. Is the dependency for a service or specification implementation that is needed at runtime? If so then the scope is `runtime`.
5. Is the dependency for a library or service API that is needed for compilation? If so then the scope is `compile`.
6. Is the dependency needed to compile or run tests for the project? If so then the scope is `test`.  

These rules are actually no different for OSGi bundles than they are for non-OSGi JARs. When creating OSGi bundles in Maven, however, it is even more important than usual to scope your dependencies correctly. Projects that incorrectly declare their dependencies are harder to deploy, and can add significant overhead to the applications that try to use them.  

## Creating a Simple OSC Plugin
Having set up a basic OSGi project we need to provide our OSC plugin implementation. 
> This tutorial refers mostly to an OSC Manager Plugin, the process is however very similar for an SDN Controller plugin. 


To implement a plugin we need to make the OSC Plugin API dependency available as a Maven dependency:
...
Note that the **API** dependency is pulled in as a `provided` scope dependency. This is because at runtime the SDK should be provided by the OSC server (step 1 of the scope selection process).  
The manager plugin implementation can be created automatically in the IDE, and will look something like this:  
``` java
...
The various methods of the API should be completed as appropriate, and will likely involve creating
other classes that implement parts of the OSC SDK.  
...
Declarative Services descriptors can be written by hand, but the simplest way to use Declarative Services is to generate the metadata from the standard annotations. The `bnd-maven-plugin` already processes these annotations by default, so all that is required is to add the dependency:
...
Deactivate methods should not block or steal the incoming thread as this risks deadlocking the system. Furthermore deactivate methods should halt any threads and close any resources held by the component. Failing to do this can cause leaks in the system, as no other actor is responsible for cleaning up these resources.
...
* Require-Bundle — Provides a list of bundles that should be resolved and deployed using the index. This header has the same syntax as the standard Require-Bundle header for an OSGi bundle. This list is turned into a set of OSGi requirements and added to the list defined by `Require-Capability`.
* Require-Capability — Provides a list of generic capabilities that should be resolved against the index and deployed.

See below how the plugin can be packaged using the `maven-antrun-plugin`
...
```
Be careful if you want to use ${project.version} in the Deployment-Version header. Most Maven release versions are compatible with OSGi version syntax, but SNAPSHOT versions are not. A simple regular expression can fix up the version if needed. The following two examples demonstrate how to set an `osgi.version` property which works with SNAPSHOT versions.  
...
Once the output file has been created it needs to be attached as an output of the project using the `build-helper-maven-plugin`:
```xml
<plugin>
    <groupId>org.codehaus.mojo</groupId>
    <artifactId>build-helper-maven-plugin</artifactId>
